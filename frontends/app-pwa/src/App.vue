...
const page = ref('home'), loading = ref(false), error = ref('')
...
<template>
<main class="shell">
  <section v-if="!token" class="login-screen"><img class="brand-mark" src="/icons/almunjaz.png" alt="شعار المنجز"><h1>المنجز السريع</h1><p>منصة التوصيل للتاجر والمندوب</p><form @submit.prevent="submitLogin" class="login-card"><div class="roles"><button type="button" :class="{active:login.role==='merchant'}" @click="login.role='merchant'">🏪<b>تطبيق التاجر</b><small>طلباتي ومحفظتي</small></button><button type="button" :class="{active:login.role==='courier'}" @click="login.role='courier'">🏍️<b>تطبيق المندوب</b><small>توصيلاتي وتحصيلاتي</small></button></div><label>اسم المستخدم<input v-model="login.username" required autocomplete="username"></label><label>كلمة المرور<input v-model="login.password" required type="password" autocomplete="current-password"></label><p v-if="error" class="error">{{error}}</p><button class="primary" :disabled="loading">{{loading?'جارِ الدخول…':'تسجيل الدخول'}}</button><p class="hint">نسخة المعاينة: اكتب أي بيانات للدخول</p></form></section>
  <template v-else>
