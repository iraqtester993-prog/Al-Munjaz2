...
class AdminController extends Controller
{
    ...
    {
        $this->authorizeAdmin($request);
        return response()->json(['data' => User::query()->where('role', 'courier')->where('status', 'active')->orderBy('name')->get()->map(fn (User $user) => $this->userData($user))]);
    }
    ...
    {
        return ['id' => $user->id, 'name' => $user->name, 'username' => $user->username, 'phone' => $user->phone, 'role' => $user->role, 'status' => $user->status, 'vehicle' => $user->vehicle, 'is_online' => $user->is_online, 'wallet' => ['balance' => $user->wallet?->balance ?? 0, 'budget' => $user->wallet?->budget ?? 0]];
    }
}
