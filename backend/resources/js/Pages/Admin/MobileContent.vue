...
function submit() {
    if (editing.value ? !props.canUpdate : !props.canCreate) return
...
function contentRoute(action) {
    return `admin.settings.slides.${action}`
}
...
<template>
    <component :is="embedded ? 'div' : AdminShell" :title="embedded ? undefined : l('title')" :branch-mode="embedded ? undefined : branchMode" :class="{ 'slider-manager': embedded }">
        <section v-if="!embedded" class="content-heading">
            <div>
                <p class="eyebrow">{{ l('eyebrow') }}</p>
                <h2>{{ l('title') }}</h2>
                <p>{{ l('intro') }}</p>
            </div>
            <button v-if="canCreate" class="btn primary" type="button" @click="openCreate">＋ {{ l('new') }}</button>
        </section>

        <section v-else class="slider-manager-toolbar">
            <div><b>{{ l('title') }}</b><span>{{ l('intro') }}</span></div>
            <button v-if="canCreate" class="btn primary" type="button" @click="openCreate">＋ {{ l('new') }}</button>
        </section>
        ...
        <section v-if="slides.length" class="slides-grid">
            <article v-for="slide in slides" :key="slide.id" class="slide-card" :class="{ muted: !slide.is_active }">
                ...
                <footer v-if="canUpdate || canDelete">
                    <button v-if="canUpdate" class="btn secondary" type="button" @click="openEdit(slide)">{{ l('edit') }}</button>
                    <button v-if="canDelete" class="delete-button" type="button" @click="removeSlide(slide)">{{ l('delete') }}</button>
                </footer>
            </article>
        </section>
        <section v-else class="empty-state">
            <span>✦</span><b>{{ l('empty') }}</b><button v-if="canCreate" class="btn primary" type="button" @click="openCreate">{{ l('new') }}</button>
        </section>
            ...
            <form class="slide-modal" @submit.prevent="submit">
                ...
                <div class="modal-scroll">
                    ...
                    <div class="control-grid">
                        <label><span>{{ l('audience') }}</span><PopupSelect v-model="form.audience"><option value="all">{{ l('all') }}</option><option value="merchant">{{ l('merchant') }}</option><option value="courier">{{ l('courier') }}</option></PopupSelect><small>{{ l('audienceHint') }}</small></label>
                        <label><span>{{ l('scope') }}</span><PopupSelect v-model="form.branch_id"><option v-if="!branchMode" value="">{{ l('globalScope') }}</option><option v-for="branch in branches" :key="branch.id" :value="branch.id">{{ branch.name_ar || branch.name_en }}{{ branch.city ? ` — ${branch.city}` : '' }}</option></PopupSelect><small>{{ form.branch_id ? l('branchScope') : l('globalScope') }}</small></label>
