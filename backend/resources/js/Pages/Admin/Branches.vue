...
function setCityFromProvince() {
    const province = props.provinces.find((item) => String(item.id) === String(form.province_id))

    if (province) form.city = provinceName(province)
}
...
function openEdit(branch) {
    if (!props.canUpdateBranches) return
...
function submitAccess() {
    if (!props.canUpdateBranches || !accessModalBranch.value) return
...
function submit() {
    if (editing.value ? !props.canUpdateBranches : !props.canCreateBranches) return
    const options = {
        preserveScroll: true,
        onSuccess: () => {
            closeModal()
    ...
    <AdminShell :title="t('Branches')">
        ...
        <div class="branch-grid">
            <article v-for="branch in branches" :key="branch.id" class="branch-card" :class="{ inactive: !branch.is_active }">
                ...
                <div v-if="canManageBranches" class="access-summary">
                    <div>
                ...
                <div v-if="canUpdateBranches" class="branch-actions">
        ...
        <div v-if="modalOpen && (editing ? canUpdateBranches : canCreateBranches)" class="modal-backdrop" @click.self="closeModal">
            <form class="branch-modal" @submit.prevent="submit">
                ...
                <div class="form-grid">
                    ...
                    <select v-model="form.province_id" required @change="setCityFromProvince">
                ...
                <label>{{ t('City / Locality') }}<input v-model="form.city" required maxlength="60" :placeholder="t('Baghdad')" /></label>

                <aside v-if="!editing" class="auto-access-note">
                    <span aria-hidden="true">⌁</span>
                    <div>
                        <b>{{ t('Branch manager access is created automatically') }}</b>
                        <p>{{ t('After saving, a sign-in email and a temporary password are generated for this governorate. Save them when they appear; the password is shown once only.') }}</p>
                    </div>
                </aside>
        ...
        <div v-if="accessModalBranch && canUpdateBranches" class="modal-backdrop" @click.self="closeAccess">
        ...
        <div v-if="credentials && canManageBranches" class="credential-backdrop" @click.self="credentials = null">
            <section class="credentials-card" role="dialog" aria-modal="true" :aria-label="t('New branch account credentials')">
                ...
                <div class="credential-row"><span>{{ credentials.email ? t('Email') : t('Username') }}</span><b dir="ltr">{{ credentials.email || credentials.username }}</b><button type="button" @click="copyCredential(credentials.email || credentials.username)">{{ t('Copy') }}</button></div>
