...
const props = defineProps({
    ...
    canUpdateRewardSetting: { type: Boolean, default: false },
    canAdjustPoints: { type: Boolean, default: false },
...
function refresh() {
    router.get(route('admin.loyalty'), { branch_id: props.branchFilter?.selected_id || undefined }, { preserveScroll: true, replace: true })
}

function changeBranchFilter(branchId) {
    router.get(route('admin.loyalty'), { branch_id: branchId || undefined }, { preserveScroll: true, replace: true })
}

function saveSettings() {
    if (!props.canUpdateRewardSetting || settingForm.processing) return

    settingForm.post(route('admin.loyalty.settings'), { preserveScroll: true })
}

function openAdjustment(courier) {
    if (!props.canAdjustPoints) return
...
<template>
    <AdminShell :title="l('title')">
        ...
        <div class="loyalty-branch-filter"><BranchFilter :filter="branchFilter" @change="changeBranchFilter" /></div>
            ...
            <form class="panel reward-panel" @submit.prevent="saveSettings">
                ...
                <div class="reward-body">
                    <label class="reward-field">
                        ...
                        <input v-model.number="settingForm.points_per_delivery" type="number" min="0" max="1000000" step="1" inputmode="numeric" :disabled="!canUpdateRewardSetting" required />
                        <small v-if="settingForm.errors.points_per_delivery" class="field-error">{{ settingForm.errors.points_per_delivery }}</small>
                    </label>
                    <button v-if="canUpdateRewardSetting" class="primary save-reward" type="submit" :disabled="settingForm.processing"><span v-if="settingForm.processing" class="loader"></span><span v-else>{{ l('save') }}</span></button>
                </div>
            </form>

            <section v-if="canAdjustPoints" class="panel manual-panel">
                <header class="panel-head"><span class="panel-icon manual-icon">±</span><span><h3>{{ l('manual') }}</h3><p>{{ l('manualHint') }}</p></span></header>
                <div class="manual-body">
                    <label><span>{{ l('selectCourier') }}</span><PopupSelect v-model="selectedCourierId" @change="adjustmentForm.courier_id = selectedCourierId"><option value="">{{ l('chooseCourier') }}</option><option v-for="courier in couriers" :key="courier.id" :value="courier.id">{{ courier.name }} · {{ courier.phone || roleLabel(courier.role) }}</option></PopupSelect></label>
        ...
        <section class="panel courier-panel">
            ...
            <div v-if="filteredCouriers.length" class="courier-grid">
                <button v-for="courier in filteredCouriers" :key="courier.id" type="button" class="courier-card" :class="{ selected: Number(selectedCourierId) === Number(courier.id) }" :disabled="!canAdjustPoints" @click="openAdjustment(courier)">
        ...
        <div v-if="adjustmentOpen && selectedCourier && canAdjustPoints" class="dialog-backdrop" @click.self="closeAdjustment">
