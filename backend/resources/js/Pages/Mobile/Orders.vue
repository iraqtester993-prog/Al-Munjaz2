        ...
        <SheetModal :open="!!selected" @close="selected = null">
            <template v-if="selected">
                ...
                <div class="detail-row">
                    ...
                    <b class="delivery-vehicle-pill"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M21 8 12 3 3 8v8l9 5 9-5V8ZM3 8l9 5 9-5M12 13v8"/></svg>{{ orderTypeLabel(selected) }}</b>
                </div>
                ...
                <button v-if="selected.courier && !isCourier" class="order-complaint order-chat" type="button" @click="openOrderChat(selected)">
...
.order-detail-status-head{display:flex;align-items:center;gap:8px}.order-detail-status-head :deep(.badge){margin-inline-end:auto;flex:none}.order-detail-track{color:var(--primary-strong);font-size:16px;font-weight:950}.order-detail-icon{display:grid;place-items:center;width:32px;height:32px;border-radius:10px;background:var(--primary-tint);color:var(--primary-strong)}
