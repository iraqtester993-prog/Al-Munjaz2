...
import LocationPermissionSettings from '../../Components/LocationPermissionSettings.vue'
...
<template>
    <AppShell :title="t('My Profile')">
        ...
        <section class="list-card profile-detail-card" :aria-label="t('Account Details')">
            <div class="profile-detail-row"><span>{{ t('Full Name') }}</span><b>{{ profile.name || user.name || '—' }}</b></div>
            <div class="profile-detail-row"><span>{{ t('Phone Number') }}</span><b class="mono" dir="ltr">{{ profile.phone || user.phone || '—' }}</b></div>
            <div v-if="!isCourier" class="profile-detail-row"><span>{{ t('Shop Name') }}</span><b>{{ profile.shop_name || '—' }}</b></div>
            <div class="profile-detail-row"><span>{{ isCourier ? t('Address') : t('Shop Address') }}</span><b>{{ profile.address || '—' }}</b></div>
            <div class="profile-detail-row"><span>{{ t('Governorate') }}</span><b>{{ provinceName }}</b></div>
            <div v-if="isCourier" class="profile-detail-row"><span>{{ t('Vehicle') }}</span><b>{{ vehicleName }}</b></div>
            <div v-if="!isCourier" class="profile-detail-row"><span>{{ t('Account Verification') }}</span><b class="profile-verification-state" :class="verificationState">{{ verificationStatusLabel(verificationState) }}</b></div>
            <div class="profile-detail-row"><span>{{ t('Joined') }}</span><b class="mono" dir="ltr">{{ profile.joined_at || '—' }}</b></div>
        </section>
        ...
        <section class="profile-permissions" :aria-label="t('Device permissions')">
            ...
            <LocationPermissionSettings :user-id="user.id" :share-courier-location="isCourier" />
        </section>
        ...
        <section class="list-card profile-actions">
            ...
            <button v-else class="settings-row clickable" type="button" @click="showDocuments = true"><div class="sri"><svg viewBox="0 0 24 24"><path :d="icon('shield')" /></svg></div><div class="srt">{{ t('Documents') }}</div><span class="srv">›</span></button>
        ...
        <SheetModal :open="showDocuments" :title="t('Documents')" @close="showDocuments = false">
