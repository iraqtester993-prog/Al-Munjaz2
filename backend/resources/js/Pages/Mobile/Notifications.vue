...
function meta(t) {
    return typeMeta.value[t] || { tint: 'var(--surface-2)', color: 'var(--ink-soft)', icon: 'M12 5v14M5 12h14' }
}

function markAll() {
    router.post(route('app.notifications.read-all'), {}, { preserveScroll: true })
}
</script>

<template>
    <AppShell :title="t('Notifications')" :notif-badge="unread" :show-notif="false">
        <button v-if="unread > 0" class="btn btn-ghost" style="width: 100%; margin-bottom: 12px" @click="markAll">
            {{ t('Mark all as read') }}
        </button>

        <div v-if="notifications.length" class="list-card">
        ...
        <div v-else class="empty-hint">{{ t('No notifications yet') }}</div>
    </AppShell>
</template>
